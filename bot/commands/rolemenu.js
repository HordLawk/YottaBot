...
module.exports = {
    ...
    execute: async function(message){
        ...
        if(args[0] === 'create'){
            ...
            if(!message.guild.me.permissionsIn(discordChannel).has(Permissions.FLAGS.ADD_REACTIONS)) return message.channel.send(channelLanguage.get('botReactions'));
            ...
            if(!message.guild.me.permissionsIn(discordChannel).has(Permissions.FLAGS.SEND_MESSAGES)) return message.channel.send(channelLanguage.get('sendMessages'));
            if(!message.guild.me.permissionsIn(discordChannel).has(Permissions.FLAGS.ADD_REACTIONS)) return message.channel.send(channelLanguage.get('botReactions'));
            if(!message.guild.me.permissionsIn(discordChannel).has(Permissions.FLAGS.EMBED_LINKS)) return message.channel.send(channelLanguage.get('botEmbed'));
            if(!message.guild.me.permissionsIn(discordChannel).has(Permissions.FLAGS.MANAGE_MESSAGES)) return message.channel.send(channelLanguage.get('botManageMessages'));
            let roles = args.slice(2).filter((e, i) => ((i % 2) === 0)).map(e => (message.guild.roles.cache.get(e.match(/^(?:<@&)?(\d{17,19})>?$/)?.[1]) ?? message.guild.roles.cache.find(ee => (ee.name.toLowerCase() === e.toLowerCase().replace(/"/g, ''))) ?? message.guild.roles.cache.find(ee => (ee.name.toLowerCase().startsWith(e.toLowerCase().replace(/"/g, '')))) ?? message.guild.roles.cache.find(ee => (ee.name.toLowerCase().includes(e.toLowerCase().replace(/"/g, ''))))));
            if(roles.some(e => (!e || (e.id === message.guild.id)))) return message.channel.send(channelLanguage.get('invArgs', [message.client.guildData.get(message.guild.id).prefix, this.name, this.usage(channelLanguage)]));
            if(roles.some(e => (!e.editable || e.managed))) return message.channel.send(channelLanguage.get('manageRole'));
            if(roles.some(e => (e.position >= message.member.roles.highest.position))) return message.channel.send(channelLanguage.get('memberManageRole'));
