...
module.exports = {
    ...
    execute: async function(message){
        ...
        if(args[0] === 'create'){
            ...
            let emojis = args.slice(2).filter((e, i) => ((i % 2) != 0)).map(e => (message.guild.emojis.cache.get(e.match(/^(?:<:\w+:)?(\d{17,19})>?$/)?.[1]) ?? message.guild.emojis.cache.find(ee => (ee.name.toLowerCase() === e.toLowerCase())) ?? message.guild.emojis.cache.find(ee => ee.name.toLowerCase().startsWith(e.toLowerCase())) ?? message.guild.emojis.cache.find(ee => ee.name.toLowerCase().includes(e.toLowerCase())) ?? parse(e)[0]?.text));
