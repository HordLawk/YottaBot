...
module.exports = {
    ...
    execute: async function(message){
        ...
        if(args[0] === 'create'){
            ...
            let roles = args.slice(2).filter((e, i) => ((i % 2) === 0)).map(e => (message.guild.roles.cache.get(e.match(/^(?:<@&)?(\d{17,19})>?$/)?.[1]) ?? message.guild.roles.cache.find(ee => (ee.name === e.replace(/"/g, ''))) ?? message.guild.roles.cache.find(ee => ee.name.startsWith(e.replace(/"/g, ''))) ?? message.guild.roles.cache.find(ee => ee.name.includes(e.replace(/"/g, '')))));
            ...
            let emojis = args.slice(2).filter((e, i) => ((i % 2) != 0)).map(e => (message.guild.emojis.cache.get(e.match(/^(?:<:\w+:)?(\d{17,19})>?$/)?.[1]) ?? message.guild.emojis.cache.find(ee => (ee.name === e)) ?? message.guild.emojis.cache.find(ee => ee.name.startsWith(e)) ?? message.guild.emojis.cache.find(ee => ee.name.includes(e)) ?? parse(e)[0]?.text));
            if(emojis.some(e => (!e || (e.id && (!e.available || e.managed))))) return message.channel.send(channelLanguage.get('invArgs', [message.client.guildData.get(message.guild.id).prefix, this.name, this.usage(channelLanguage)]));
            if(emojis.some(e => (emojis.filter(ee => ((ee.id || ee) === (e.id || e))).length > 1))) return message.channel.send(channelLanguage.get('uniqueEmoji'));
            let loadmsg = await message.channel.send(channelLanguage.get('loading'));
