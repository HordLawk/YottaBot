...
module.exports = {
    ...
    execute: async function(message, args){
        ...
        await current.save();
        await member.user.send(`You were warned in **${message.guild.name}**${reason ? `with the reason: *${reason}*` : ''}`).catch(() => message.channel.send('The warn couldn\'t be DMed to the user. This usually happens when a user disables DMs for this server'));
        ...
        const embed = new MessageEmbed()
            .setTimestamp()
            .setColor(0xffff00)
            .setAuthor(`${message.author.tag} warned ${member.user.tag}`, member.user.avatarURL({dynamic: true}))
