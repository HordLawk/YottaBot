...
fs.readdirSync(path.join(__dirname, 'events')).filter(file => file.endsWith('.js')).map(e => require(`./events/${e}`)).forEach(e => client.on(e.name, (...args) => {
    try{
        e.execute(...args, client)
    }
    catch(error){
        console.error(error);
        console.log(e.name);
        console.log(args);
        client.channels.cache.get(client.configs.errorlog).send(`Error: *${error.message}*\nEvent: ${e.name}\nArgs:\n${args.map(arg => arg.toString()).join('\n')}`).catch(console.error);
    }
}));
(async () => {
